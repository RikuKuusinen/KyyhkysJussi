 ...
 *  - A trivial form handler at /formhandler  using POST
 ...
 *  - If package.json file is present:
 *         npm install
 *  - without package.json:
 * 	       npm install install express body-parser cors
 * 
 * Using:
 *  - Start the server using one or the other:
 * 		   npm start
 * 		   node labserver.js
...
app.post('/formhandler', parser, function (req, res) {
   var respdata = "<!DOCTYPE html><html><head><meta charset='utf-8'/><title>Formdata</title></head>"
   respdata += "<body><h1>Data received</h1>";
   respdata += "<table border=1><tr><th>Field name</th><th>Value</th></tr>";
   for (var fname in req.body) {
	   respdata += "<tr><td>"+fname+"</td><td>"+decodeURIComponent(req.body[fname])+"</td></tr>"; 
	}
   respdata += "</table>";
   respdata += "</body></html>";
   res.end(respdata);
