...
namespace Harjoitus9_1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Jumppa.lajitteluKriteeri = "nimi";
            lista.Sort();

            for (int i = 0; i < 5; i++)
                Console.WriteLine(lista[i].ToString());

            Queue jono = new Queue(lista);

            jono.Enqueue(j1.nimi);
            jono.Enqueue(j2.nimi);
            jono.Enqueue(j3.nimi);
            jono.Enqueue(j4.nimi);
            jono.Enqueue(j5.nimi);

            IEnumerator enumerator = jono.GetEnumerator();
            ...
            IEnumerator enumerator2 = pino.GetEnumerator();
